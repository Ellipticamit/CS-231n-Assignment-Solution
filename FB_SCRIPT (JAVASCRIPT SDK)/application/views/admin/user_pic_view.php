	...
	if(!empty($user_pic_query)) {		
		foreach($user_pic_query as $row) {		
			$user_data .= '<tr>						
								...
								<td>'.$row->img_shape.'</td>
								<td>'.$row->img_rank.'</td>	
								<td><a href="'.site_url('admin/user/edit_user_photo/').'/'.$row->usid.'" class="btn btn-xs btn-primary">Edit</a> &nbsp; <a href="'.site_url('admin/user/delete_user_photo/').'/'.$row->usid.'_'.$id.'" class="btn btn-xs btn-danger">Delete</a></td>		
								</tr>';			
								$i++;		
		}	
	}
		...
		<section class="vbox"> 			
			<section class="scrollable padder"> 			
				<ul class="breadcrumb no-border no-radius b-b b-light pull-in"> 	
					...
					<li class="active">Add User Pics Details</li>
				</ul>	
				<section class="panel panel-default"> 		
					<header class="panel-heading font-bold">Users FB Pic Details</header> 					
						<div class="panel-body"> 		
							<?php					
								...
								if(!empty($success)) { 		
								 ...
								 echo form_open('admin/user/user_photo_details/'.$id, $attributes);
							?>		
								...
								<div class="col-md-3 form-group" style="padding:0px 10px 0px 0px;"> 								
									...
									<input class="form-control" placeholder="100" type="text" name="img_rank"> 								
