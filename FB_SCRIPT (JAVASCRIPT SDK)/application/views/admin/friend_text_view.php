	...
	if(!empty($name_query)) {
		foreach($name_query as $row) {
			$text_data .= '<tr>
							...
							<td>'.$row->text_y.'</td>
							<td>'.$row->options.'</td>
							<td><a href="'.site_url('admin/friend/edit_friend_text/').'/'.$row->nid.'" class="btn btn-xs btn-primary">Edit</a> &nbsp;
								<a href="'.site_url('admin/friend/delete_friend_text/').'/'.$row->nid.'_'.$row->appid.'" class="btn btn-xs btn-danger">Delete</a></td>
						  </tr>';;
			$i++;
		}
	}
		...
		<section class="vbox"> 
			<section class="scrollable padder"> 
				<ul class="breadcrumb no-border no-radius b-b b-light pull-in"> 
					...
					<li class="active">Add Friend Name Details</li>						
				</ul>
				
				<section class="panel panel-default"> 
					<header class="panel-heading font-bold">Friend Name Details</header> 
				...
				<?php	
					...
					echo form_open('admin/friend/friend_text/'.$id, $attributes); 
				?>
							...
							<div class="form-group col-md-12" style="padding:0px 10px 0px 0px;"> 
								...
								<textarea name="app_text" class="form-control col-md-12" rows="3" placeholder="$name. your text or your $name text
								"></textarea> 
						    ...
						    <div class="form-group col-md-4"> 
								<label>Full Name Options</label>				
								<div class="checkbox" style="margin-top:0px;"> <label> 
									<input type="checkbox" value="yes" name="full_name"> Yes  </label> 
								</div>
						    </div>
